...
use Bitrix\Main\Diag\Helper; // Импорт класса Helper
...
foreach($sender as $email_check) {
	...
	$items = \CCrmFieldMulti::GetListEx(["ELEMENT_ID" => "DESC"], [
		'VALUE' => $email_check,
		'TYPE_ID' => 'EMAIL'
	], false, false, [], []);
	...
	if($arContact) {
		...
		foreach($dealContact as $dealId) {
			$dbDeal = \CCrmDeal::GetListEx(
				...
				false,
				false,
				['ID','LEAD_ID', "DATE_CREATE", "ASSIGNED_BY_ID", 'UF_CRM_6318FC326D01B', 'UF_CRM_6318FC334D52D']
			);
			if($res = $dbDeal->Fetch()) {
				$arDeal = $res;
...
/* 			if(!$arDeals) {
		...
		if(!empty($arDeal)) {
			\Bitrix\Main\Diag\Debug::dumpToFile($arDeal["ID"], 'create new lead from old DEAL', $logPath);

			//Если найдена открытая сделка не текущей даты, создаём лид
			if ($dealDateTime < $date) {
				$data['ownerId'] = $arDeal["ID"];
				$data['ownerTypeId'] = \CCrmOwnerType::Deal;
				AddActivityDbbo($message, $data);

				$leadId = addLeadFromEmail($message,$arDeal["ASSIGNED_BY_ID"],$email_check);
				resendEmail($message,$arDeal["ASSIGNED_BY_ID"],$email_check,$leadId,\CCrmOwnerType::Lead);
				\Bitrix\Main\Diag\Debug::dumpToFile($leadId, 'create new LEAD from old deal', $logPath);
			}
			else {
				...
				\Bitrix\Main\Diag\Debug::dumpToFile($arDeal["ID"], 'add into deal', $logPath);
				...
				if($arDeal['LEAD_ID']) {
					\Bitrix\Main\Diag\Debug::dumpToFile($arDeal['LEAD_ID'], 'add into lead of deal', $logPath);
					
					$data['ownerId'] = $arDeal['LEAD_ID'];
			...
			//lead
			//Поиск лидов по различным кейсам (Проверка при условии что Нет открытой сделки), крепим письмо к лиду. Проверяем ответственного: Колганов, не отправляем письмо. Другой менеджер, отправляем письмо
				$findLead = false;
			...
			if(!$findLead) {
				$dbRes = \CCrmLead::GetListEx(
					...
					false,
					false,
					['ID','ASSIGNED_BY_ID','UF_CRM_1680466157','DATE_CLOSED']
				);
				if($res = $dbRes->Fetch()) {
					if ($res["ASSIGNED_BY_ID"] != 1152 && $res["ASSIGNED_BY_ID"] != 46) {			
						if($res["UF_CRM_1680466157"] && $res['DATE_CLOSED']) {	
							...
							if($currentTimeStart == $dealTimeStart) {

								$query = \Bitrix\Crm\StatusTable::query()
									->setSelect(["STATUS_ID"])
									->setFilter(["NAME" => $res["UF_CRM_1680466157"]])
									->exec();
								$statusId = $query->fetch()["STATUS_ID"]; 
...
/* 					SendEmailBP($messageId, 114, 'CCrmDocumentLead', 'LEAD_'. $leadId, [
		...
		if($res = $dbRes->Fetch()) {
			...
			// Есть открытый лид и контакт не привязан к лиду и ответственный не Евгений Колганов
			if ($res["ASSIGNED_BY_ID"] != 16) {
				resendEmail($message,$res["ASSIGNED_BY_ID"],$email_check,$res["ID"],\CCrmOwnerType::Lead);
			}
		...
		if (!$findLead) {
			...
			if($res = $dbRes->Fetch()) {
				if ($res["ASSIGNED_BY_ID"] != 1152 && $res["ASSIGNED_BY_ID"] != 46) {
					//Береём предыдущий статус лида и проверяем дату закрытия лида с текущей датой		
					if($res["UF_CRM_1680466157"] && $res['DATE_CLOSED']) {	
						$leadClosedDateTime = new \Bitrix\Main\Type\DateTime($res["DATE_CLOSED"]);
						$dealTimeStart = new \Bitrix\Main\Type\DateTime($leadClosedDateTime->format('d.m.Y'));
						$currentDateTime = new \Bitrix\Main\Type\DateTime();
						$currentTimeStart = new \Bitrix\Main\Type\DateTime($currentDateTime->format('d.m.Y'));
						...
						if($currentTimeStart == $dealTimeStart) {
							...
							$entity = new \CCrmLead(false);
							$updateFields = [
								'STATUS_ID' => $res["UF_CRM_1680466157"]
							];
							$entity->Update($res['ID'], $updateFields, true, true, []);
							...
							// Есть открытый лид и контакта нет в базе и ответственный не Евгений Колганов тогда пересылаем письмо ответственному за лид
							if ($res["ASSIGNED_BY_ID"] != 16) {
								resendEmail($message,$res["ASSIGNED_BY_ID"],$email_check,$res["ID"],\CCrmOwnerType::Lead);
							}
...
//Логируем вызовы
$logFilePath = "/local/logs/mail/mailManager" . date("Y-m-d") . ".log";
