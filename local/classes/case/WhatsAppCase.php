...
class WhatsAppActivity
{
    ...
    public function createActivity(bool $isCompleted = false, string $status = 'sent')
    {
        ...
        switch (strtolower($status)) { // Приводим статус к нижнему регистру
            case 'sent':
                $statusType = 'secondary';
                $statusTitle = 'Отправлено';
                break;
        ...
        if (!$layoutDto->hasValidationErrors()) {
            ...
            if ($saveResult->isSuccess()) {
                $activityId = $saveResult->getData()['id'];
                \CCrmActivity::Update($activityId, ['COMPLETED' => 'Y']);
                return $activityId;
            } else {
...
// Класс для получения информации о клиенте
class ContactInfo
{
    /**
