...
if (!empty($arResult['LIST']))
{
	...
	foreach ($arResult['LIST'] as $key => $row)
	{
		...
		//hsfix start
		$color = "";
		switch ($preparedRow['content']['UF_CRM_TYPE']) {
			case "Проект":
				$color = "skyblue";
				break;
			case "Лид":
...
$arResult["FILTER"]["UF_CRM_TYPE"]["type"] = "list";
$arResult["FILTER"]["UF_CRM_TYPE"]["items"] = $types;

foreach ($arResult['ROWS'] as $key => $row) {
	if ($row["columns"]["STATUS"] == "Завершена" && $row["counters"]["ACTIVITY_DATE"]["value"] > 0) {
		if (\Hs\Tasks::deletePings($row["id"])) {
