...
<div class="task-detail">
	...
	<div class="task-detail-buttons"><?
		//hsfix start
			if (!CSite::InGroup(CRM_SETTINGS["tasks"]["enableSystemTaskButtonsUserGroups"]) && !empty($arResult["AUX_DATA"]["USER_FIELDS"]["UF_CRM_TYPE"]["NAME"]) && !in_array($arResult["AUX_DATA"]["USER_FIELDS"]["UF_CRM_TYPE"]["NAME"],CRM_SETTINGS['tasks']['enableSystemTaskButtonsCrmTypes']) && !$USER->IsAdmin()) {
			} else {
				$APPLICATION->IncludeComponent(
...
<?php
//Скрываем кнопки в задаче
 // Текущее значение ID

if (!in_array(46, $USER->GetUserGroupArray()) && !$USER->IsAdmin()) {
    $allowedIds = [2900, 2901, 2904];
    $entityId = $arResult["AUX_DATA"]["USER_FIELDS"]["UF_CRM_TYPE"]['VALUE'];

    if (in_array($entityId, $allowedIds)) {
//        writeLog('Task', 'Открыта задача лида', $entityId);

        // Подключение пользовательских стилей
        $this->addExternalCss($this->GetFolder() . '/hide-all-on-lock.css');
    } else {
        $this->addExternalCss($this->GetFolder() . '/hide-extra-elements.css');
    }
} else {
    $this->addExternalCss($this->GetFolder() . '/hide-extra-elements.css');
}
