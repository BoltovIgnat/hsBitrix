...
if (!empty($arParams["HID"])) {
	...
	foreach ($arResult['GRID_DATA'] as &$griddata) {
		$griddata["columns"]["SETMAIN"] = '<a href="javascript:;" onclick="setMain(this,'.$griddata["id"].','.$arParams["HID"].')">Основная</a>';
		$griddata["columns"]["UNLINK"] = '<a href="javascript:;" onclick="unlinkHolding(this,'.$griddata["id"].','.$arParams["HID"].')">Отвязать</a>';
	}
