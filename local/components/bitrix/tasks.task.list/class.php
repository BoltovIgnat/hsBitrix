...
private static function setUserData(array $list)
{
	...
	foreach ($list as $id => $row)
	{
		...
		if (!empty($list[$id]["UF_CRM_COMPANY"])) {
			$query = \Bitrix\Crm\CompanyTable::query()
			->setSelect(["ID","TITLE","COMPANY_TYPE"])
			->setFilter([ "ID" => $list[$id]["UF_CRM_COMPANY"] ])
			->exec();
			$list[$id]["UF_CRM_COMPANY"] = $query->fetch()["TITLE"];
		}
		if (!empty($list[$id]["UF_CRM_CONTACT"])) {
			$query = \Bitrix\Crm\ContactTable::query()
			->setSelect(["ID","FULL_NAME"])
			->setFilter([ "ID" => $list[$id]["UF_CRM_CONTACT"] ])
			->exec();
			$list[$id]["UF_CRM_CONTACT"] = $query->fetch()["FULL_NAME"];
		}
		if (!empty($list[$id]["UF_CRM_SUPPORT"])) {
			$factory = \Bitrix\Crm\Service\Container::getInstance()->getFactory(CRM_SMART["support"]);
			$support = $factory->getItems([
				'filter' => ["ID" => $list[$id]["UF_CRM_SUPPORT"]],
				'select' => ['TITLE'],
				'order' => ['ID' => 'ASC'],
			]);

			$list[$id]["UF_CRM_SUPPORT"] = $support[0]["TITLE"];
		}

		if (!empty($list[$id]["UF_CRM_TYPE"])) {
			$list[$id]["UF_CRM_TYPE"] = $types[$list[$id]["UF_CRM_TYPE"]];
		}
	}
