...
elseif(!isset($arSort['nearest_activity']))
{
    ...
    //hsfix start
	$navListOptions["IS_EXTERNAL_CONTEXT"] = "N";
	$arFilter['CHECK_PERMISSIONS'] = 'N';
	 foreach ($arFilter as $key => $value) {
		if ($key != "CHECK_PERMISSIONS" && str_contains($key,"PARENT") == false) {
			unset($arFilter[$key]);
		}
	}

/*     if (!empty($arParams["INTERNAL_FILTER"]["HOLDING_ID"])) {
        $query = \Bitrix\Crm\Relation\EntityRelationTable::query()
            ->addSelect("DST_ENTITY_ID")
            ->setFilter([
                "SRC_ENTITY_TYPE_ID" => CRM_SMART["holding"],
                "SRC_ENTITY_ID" => $arParams["INTERNAL_FILTER"]["HOLDING_ID"],
        ...
        foreach ($companies as $company) {
            $companyIds[] = $company["DST_ENTITY_ID"];
        }
		...
		$query = \Bitrix\Crm\Relation\EntityRelationTable::query()
			...
			->setFilter([
				"SRC_ENTITY_TYPE_ID" => CRM_SMART["holding"],
				"SRC_ENTITY_ID" => $arParams["INTERNAL_FILTER"]["HOLDING_ID"],
				"DST_ENTITY_TYPE_ID" => \CCrmOwnerType::Contact
			])
			->exec();
		$contactsRel = $query->fetchAll();

		foreach ($contactsRel as $contact) {
			$contactIdsRel[] = $contact["DST_ENTITY_ID"];
		}

		//deal for companies
		$arFilter["COMPANY_ID"] = $companyIds;
		$dbResult = CCrmDeal::GetListEx(
			$arSort,
			$arFilter,
			false,
		...
		unset($arFilter["COMPANY_ID"]);
		$arFilter["CONTACT_ID"] = ($contactIdsRel) ? array_merge($contactsIds,$contactIdsRel) : $contactsIds;
...
foreach($arResult['DEAL'] as &$arDeal)
{
	...
	if ($arResult['ENABLE_TASK'])
	{
		$arDeal['PATH_TO_TASK_EDIT'] = CHTTP::urlAddParams(
			...
			array(
				...
				'TITLE' => urlencode(GetMessage('CRM_TASK_TITLE_PREFIX') . ' component.php'),
				'TAGS' => urlencode(GetMessage('CRM_TASK_TAG')),
				'back_url' => urlencode($arParams['PATH_TO_DEAL_LIST'])
			)
		);
	...
	$arDeal['ASSIGNED_BY'] = htmlspecialcharsbx($arDeal['ASSIGNED_BY_NAME'].' '. $arDeal['ASSIGNED_BY_LAST_NAME']); 
...
else
{
	...
	else
	{
		...
		if (defined('BX_UTF') && BX_UTF)
			echo component . phpchr(239) . chr(187) .chr(191);
