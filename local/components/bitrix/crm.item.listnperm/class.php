...
class CrmItemListComponent extends Bitrix\Crm\Component\ItemList
{
	...
	protected function prepareGrid(array $listFilter, PageNavigation $pageNavigation, array $gridSort): array
	{
		...
		{
//hsfix start
			unset($listFilter["FILTER_APPLIED"]);
			unset($listFilter["PRESET_ID"]);
			unset($listFilter["FILTER_ID"]);
			unset($listFilter["SEARCH_CONTENT"]);
//hsfix end
			$order = $this->validateOrder($gridSort['sort']);
			$list = $this->factory->getItems(
				[
			...
			$totalCount = $this->factory->getItemsCount($listFilter);
		}

        if(!defined('SHOW_GRID_SETTINGS_MENU_USERS')) {
            define('SHOW_GRID_SETTINGS_MENU_USERS', 44);
        }
        global $USER;
        $group = $USER->GetUserGroupArray();
        $isSettings = false;
        if (in_array(SHOW_GRID_SETTINGS_MENU_USERS, $group)) {
            $isSettings = true;
        }
		...
		$grid['SHOW_GRID_SETTINGS_MENU'] = $isSettings;
