...
foreach($arResult['COMPANY'] as $sKey =>  $arCompany)
{
	...
	$resultItem = array(
		...
		'columns' => array(
			...
			'CURRENCY_ID' =>  CCrmCurrency::GetEncodedCurrencyName($arCompany['CURRENCY_ID']),
...
if (!empty($arParams["HID"])) {

	$button["SETMAIN"] = 
	[
		"id" => "SETMAIN",
		"name" => "Сделать основной",
		"default" => true,
		"editable" => false,
		"type" => "custom",
	];
	...
	array_push($arResult['HEADERS'],$button["SETMAIN"]);
	array_push($arResult['HEADERS'],$button["UNLINK"]);

	foreach ($arResult['GRID_DATA'] as &$griddata) {
		if ($griddata["id"] != $arParams["COMPANY_ID"]) {
			$griddata["columns"]["SETMAIN"] = '<a href="javascript:;" style="font-size:14px;margin: -8px 25px 0px 0px;" class="linkbutton" onclick="setMain(this,'.$griddata["id"].','.$arParams["HID"].')">Установить как основную</a>';
		}
		else {
			$griddata["columns"]["SETMAIN"] = '<span class="graybutton"><b>ОСНОВНАЯ</b></span>';
		}
		$griddata["columns"]["UNLINK"] = '<a href="javascript:;" class="unlinkbutton" onclick="unlinkHolding(this,'.$griddata["id"].','.$arParams["HID"].')">Отвязать</a>';
	}
