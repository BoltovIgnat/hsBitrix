...
/* 		if (!empty($_REQUEST["holdingID"])) {
			...
			->setFilter(["CONTACT_ID" => $contactIdsRel])
			->exec();
			$companyRes = $query->fetchAll();
			foreach ($companyRes as $company) {
				$companyIdsrel[] = $company["COMPANY_ID"];
			}
			$arFilter["@ID"] = ($companyIdsrel) ? array_merge($companyIdsrel,$companyIds) : $companyIds;
		} */
