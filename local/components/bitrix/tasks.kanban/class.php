...
//		);
	...
	protected function fillData(CTaskItem $task)
	{
		...
		//hsfix start
			...
			$rsEnum = $obEnum->GetList(array(), array("USER_FIELD_ID" => CRM_SETTINGS["tasks"]["uf_crm_type"]));
			$types = array();
		...
		try
		{
			$item = $task->getData();
			//hsfix start
				$itemuf = $task->getData(array('UF_#'));
				if (!empty($itemuf["UF_CRM_TYPE"])) {
					$uf_crm_type = $types[$itemuf["UF_CRM_TYPE"]];
				}
				if (!empty($itemuf["UF_CRM_TOTAL"])) {
					$uf_crm_total = CurrencyFormat($itemuf["UF_CRM_TOTAL"], 'RUB');
				}
				if (!empty($itemuf["UF_CRM_COMPANY"])) {
					$query = \Bitrix\Crm\CompanyTable::query()
					->setSelect(["ID","TITLE","COMPANY_TYPE"])
					->setFilter([ "ID" => $itemuf["UF_CRM_COMPANY"] ])
					->exec();
					$company = $query->fetch();
				}
				if (!empty($itemuf["UF_CRM_CONTACT"])) {
					$query = \Bitrix\Crm\ContactTable::query()
					->setSelect(["ID","FULL_NAME"])
					->setFilter([ "ID" => $itemuf["UF_CRM_CONTACT"] ])
					->exec();
					$contact = $query->fetch();
				}
				if (!empty($itemuf["UF_CRM_SUPPORT"])) {
					$factory = \Bitrix\Crm\Service\Container::getInstance()->getFactory(CRM_SMART["support"]);
					$support = $factory->getItems([
						'filter' => ["ID" => $itemuf["UF_CRM_SUPPORT"]],
						'select' => ['TITLE'],
						'order' => ['ID' => 'ASC'],
					]);

					$supportName = $support[0]["TITLE"];
				}
			//hsfix end
		}
		...
		$data = array(
			//hsfix start
			'uf_crm_type' => $uf_crm_type,
			'uf_crm_total' => $uf_crm_total,
			'uf_crm_contact' => $contact["FULL_NAME"],
			'uf_crm_contact_id' => $contact["ID"],
			'uf_crm_company' => $company["TITLE"],
			'uf_crm_company_id' => $company["ID"],
			'uf_crm_company_type' => $company["COMPANY_TYPE"],
			'uf_crm_support_name' => $supportName,
			//hsfix end
			// base
			'id' => $item['ID'],
	...
	protected function getData(array $additionalFilter = [], $skipCommonFilter = false)
	{
		...
		//hsfix start
			array_push($select,"UF_CRM_TYPE");
			array_push($select,"UF_CRM_TOTAL");
			array_push($select,"UF_CRM_CONTACT");
			array_push($select,"UF_CRM_COMPANY");
			array_push($select,"UF_CRM_SUPPORT");
		//hsfix end
	...
	protected function getCounters(array $items): array
	{
		...
		{
			//prToFile($items[$taskId]);
			if ($items[$taskId]['data']['completed'] == 1 && $items[$taskId]['data']['counter']['value'] > 0) {
				if (\Hs\Tasks::deletePings($items[$taskId]['data']['id'])) {
					//unset($items[$taskId]);
				}
			}
		}
